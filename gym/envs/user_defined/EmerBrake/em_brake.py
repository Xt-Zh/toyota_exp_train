...
class EmergencyBraking(gym.Env):
    def __init__(self, **kwargs):
        ...
        self.observation_space = gym.spaces.Box(np.full([2, ], -float('inf')), np.full([2, ], float('inf')),
                                                dtype=np.float32)
    ...
    def step(self, action):
        ...
        self.obs = (np.matmul(self.A, self.obs[:, np.newaxis]) + np.matmul(self.B, self.action[:, np.newaxis])).reshape(
            [-1, ])
        if self.obs[1] < 0: self.obs[1] = 0
        done = True if self.obs[0] < 0 or self.obs[1] <= 0 else False
        constraint_violation = np.where(self.obs[0]<0, -self.obs[0], np.zeros_like(self.obs[0])) # todo: problem
        # constraint_violation = -self.obs[0]
        info = dict(reward_info=dict(reward=reward, constraints=float(constraint_violation)))
        self.cstr = constraint_violation
    ...
    def _reset_init_state(self):
        ...
        if np.random.rand() < 0.9:
            v = vmax * np.random.random()
        else:
            v = 10.0 * np.random.random()
        return np.array([d, v])
